...
class GlassElement extends HTMLElement {
    ...
    // Getters y Setters para los atributos con valores por defecto
    ...
    get radius() { return this.hasAttribute('radius') ? parseInt(this.getAttribute('radius')) : 50; }
    ...
    get fullWidth() { return this.getAttribute('full-width') === 'true' || this.hasAttribute('full-width'); }
    set fullWidth(val) { if(val) this.setAttribute('full-width', 'true'); else this.removeAttribute('full-width'); }

    get autoSize() { return this.hasAttribute('auto-size'); }
    set autoSize(val) { if (val) this.setAttribute('auto-size', ''); else this.removeAttribute('auto-size'); }
    ...
    applyDynamicStyles(element) {
        ...
        if (this.autoSize) {
            ...
            let actualWidth = this.fullWidth ? window.innerWidth : Math.ceil(rect.width);
            ...
            if (this.debug) {
                ...
                element.style.boxShadow = this.hasAttribute('no-border') ? 'none' : '1px 1px 1px 0px rgba(255,255,255, 0.60) inset, -1px -1px 1px 0px rgba(255,255,255, 0.60) inset, 0px 0px 16px 0px rgba(0,0,0, 0.04)';
                element.style.border = this.hasAttribute('no-border') ? 'none' : '1px solid rgba(255, 255, 255, 0.3)';
            } else {
                ...
                element.style.background = this.backgroundColor;
                element.style.boxShadow = this.hasAttribute('no-border') ? 'none' : '1px 1px 1px 0px rgba(255,255,255, 0.60) inset, -1px -1px 1px 0px rgba(255,255,255, 0.60) inset, 0px 0px 16px 0px rgba(0,0,0, 0.04)';
                if(this.hasAttribute('no-border')) element.style.border = 'none';
            }
        } else {
    ...
    render() {
        this.shadowRoot.innerHTML = `
            <style>
                :host {
                    display: ${this.fullWidth ? 'block' : (this.autoSize ? 'inline-block' : 'block')};
                    ${this.fullWidth ? 'width: 100%;' : ''}
                }
                
                .glass-box {
                    background: rgba(255, 255, 255, 0.4);
                    box-shadow: ${this.hasAttribute('no-border') ? 'none' : '1px 1px 1px 0px rgba(255,255,255, 0.60) inset, -1px -1px 1px 0px rgba(255,255,255, 0.60) inset, 0px 0px 16px 0px rgba(0,0,0, 0.04)'};
                    cursor: pointer;
                    transition: transform 0.1s ease;
                    position: relative;
                    ${this.fullWidth ? `display: block; width: 100%; border-radius: ${this.radius}px; min-height: ${this.minHeight}px;` : 
                ...
                .content {
                    ...
                    ${(this.autoSize && !this.fullWidth) ? 'padding: var(--glass-padding, 16px 24px);' : ''}
                }
            </style>
            <div class="glass-box">
                <div class="content">
